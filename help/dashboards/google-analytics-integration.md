...
description: Aprenda a conectar Google Analytics 4 con LLM Optimizer para medir el descubrimiento basado en la IA, la participación en el sitio y los resultados comerciales en el panel de control de Tráfico de referencia.
...
La integración de Google Analytics 4 (GA4) conecta LLM Optimizer con los datos de GA4 de su organización para que pueda medir cómo el descubrimiento impulsado por IA en plataformas como ChatGPT, Gemini, Copilot, Claude y Perplexity se traduce en participación real en el sitio web y resultados comerciales.Después de conectar una propiedad de GA4, LLM Optimizer extrae las métricas de tráfico de referencia, participación y conversión que GA4 atribuye a esas fuentes y las muestra en el panel **Tráfico de referencia** en la pestaña **Impacto en la empresa**.

>[!IMPORTANT]
>
>La integración de GA4 está incluida en la oferta de pago de LLM Optimizer.Los clientes que utilicen la versión de prueba gratuita no podrán conectar GA4 hasta que actualicen a una oferta de pago.

## Antes de empezar {#before-you-begin}

Para completar la conexión, necesita lo siguiente:

* Una cuenta de Google con al menos acceso de **Visualizador** en la propiedad de GA4 a la que desea conectarse. El acceso de nivel de propiedad se administra en Google Analytics en **Administración > Administración del acceso a propiedades**.
...
**No** tiene que crear un proyecto de Google Cloud, generar una cuenta de servicio, cargar una clave JSON o ingresar un ID de propiedad. LLM Optimizer intercambia la conexión a través de la pantalla de consentimiento estándar de OAuth de Google.

## Conectar GA4 con el panel de control de tráfico de referencia {#connect}
...
1. Seleccione la pestaña **Impacto en la empresa**.

   ![Panel de control Tráfico de referencia, pestaña Impacto en la empresa](/help/dashboards/assets/ga4-integration-01-business-impact-tab.png)
...
1. Seleccione **Conectar con Analytics**.LLM Optimizer lo enruta a **Configuración del cliente > Analytics**. En el selector de proveedores de Analytics, seleccione **Conectar Google Analytics 4**.
   ...
   ![Configuración del cliente, pestaña Análisis con GA4 seleccionado](/help/dashboards/assets/ga4-integration-02-analytics-ga4-picker.png)
...
>[!NOTE]
>
>El relleno puede tardar un par de horas en completarse.El panel Impacto en la empresa comienza a rellenarse progresivamente a medida que los datos llegan. No se requiere ninguna acción por su parte mientras se ejecuta el relleno.
...
LLM Optimizer pregunta a GA4 solo por las sesiones que el propio GA4 atribuye a una plataforma LLM. En la actualidad, son sesiones cuyos `sessionSourceMedium` coinciden con uno de `chatgpt`, `gemini.google.com`, `copilot.microsoft.com`, `claude` o `perplexity`. Adobe mantiene la lista de fuentes de LLM admitidas, que pueden ampliarse con el tiempo.
...
LLM Optimizer utiliza estos datos para rellenar el rendimiento de nivel de página, los desgloses de origen, las divisiones de país y dispositivo y las tendencias temporales del panel de Impacto en la empresa. No se utilizan datos para entrenar modelos ni se comparten fuera del inquilino.
...
P: ¿Está disponible la integración de GA4 durante la versión de prueba?
...
LLM Optimizer funciona con métricas agregadas de las API de datos de GA4 autorizadas por su organización, no con los datos sin procesar a nivel de evento.

P: ¿Cómo se ingieren los datos?

Su organización autoriza a LLM Optimizer a consultar la API de datos de GA4 para la propiedad seleccionada. El tráfico de referencia alineado con las fuentes de LLM se consume a través de esa API.
...
P: ¿Se almacenan los datos sin procesar a nivel de evento en LLM Optimizer?
...
Las rutas de la página de aterrizaje se ingieren como parte del informe estándar; las cadenas de consulta y el contenido de la página no se ingieren para esta integración.
...
P: Mi propiedad de GA4 está conectada, pero Impacto en la empresa está vacío. ¿Por qué?

LLM Optimizer solo extrae sesiones que el propio GA4 atribuye a una fuente o medio LLM admitido (en la actualidad: ChatGPT, Gemini, Copilot, Claude, Perplexity). Si la propiedad de GA4 aún no ha registrado sesiones de referencia de cualquiera de estas fuentes en la ventana de tiempo que se muestra, el panel estará vacío aunque la conexión esté en buen estado.
